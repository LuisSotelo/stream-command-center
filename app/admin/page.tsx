...
export default function AdminDashboard() {
  ...
  // Cambia esto temporalmente para ver el panel activo:
  const [isLive, setIsLive] = useState(true);
  const [loading, setLoading] = useState(true);

  // 1. Identificamos si eres TÚ (el Owner)
  // Asegúrate de que esta variable esté en tu .env.local
  const isOwner = 
  ...
  useEffect(() => {
    async function checkStatus() {
      try {
        const res = await fetch("/api/twitch/status"); // Crearemos este endpoint mañana
    ...
    checkStatus();
    // Poll opcional: revisar cada 1 minuto
    const interval = setInterval(checkStatus, 60000);
    return () => clearInterval(interval);
  }, []);

  const handleDiscount = async (amount: number) => {
    if (!isLive && !isOwner) return; // Doble candado
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white p-8 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Card: Mercado Libre (SOLO VISIBLE PARA TI) */}
        {isOwner && (
          <div className="bg-black/40 border border-orange-500/30 p-6 rounded-xl backdrop-blur-md shadow-[0_0_20px_rgba(234,88,12,0.1)]">
            ...
            <button 
              className="w-full py-6 bg-orange-600/20 border border-orange-500 rounded-lg hover:bg-orange-600/40 transition-all font-bold text-orange-500"
              onClick={() => alert("Mañana conectaremos esta API con ML")}
            >
