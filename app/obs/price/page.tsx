...
function PriceOverlayContent() {
  ...
  const hideLastUserTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const hideLevelUpTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const hideTriggerEventTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const delayedPriceTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const countdownIntervalRef = useRef<NodeJS.Timeout | null>(null);
  const soldOutTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    ...
    channel.bind("price-update", (data: any) => {
      if (data.newPrice) {
        ...
        if (data.levelName && data.levelName !== levelRef.current) {
          ...
          if (hideLevelUpTimeoutRef.current) {
            clearTimeout(hideLevelUpTimeoutRef.current);
          }

          hideLevelUpTimeoutRef.current = setTimeout(() => {
        ...
        if (delayedPriceTimeoutRef.current) {
          clearTimeout(delayedPriceTimeoutRef.current);
        }

        delayedPriceTimeoutRef.current = setTimeout(() => {
        ...
        if (hideLastUserTimeoutRef.current) {
          clearTimeout(hideLastUserTimeoutRef.current);
        }

        hideLastUserTimeoutRef.current = setTimeout(() => {
          setLastUser(null);
        }, 5000);
        ...
        if (data.specialEvent) {
          ...
          if (hideTriggerEventTimeoutRef.current) {
            clearTimeout(hideTriggerEventTimeoutRef.current);
          }

          hideTriggerEventTimeoutRef.current = setTimeout(() => {
    ...
    channel.bind("start-countdown", (data: any) => {
      ...
      if (soldOutTimeoutRef.current) {
        clearTimeout(soldOutTimeoutRef.current);
      }

      soldOutTimeoutRef.current = setTimeout(() => {
        setIsFinished(true);
      }, (totalSeconds + 1) * 1000);
    });
    ...
    return () => {
      ...
      if (hideLastUserTimeoutRef.current) clearTimeout(hideLastUserTimeoutRef.current);
      if (hideLevelUpTimeoutRef.current) clearTimeout(hideLevelUpTimeoutRef.current);
      if (hideTriggerEventTimeoutRef.current) clearTimeout(hideTriggerEventTimeoutRef.current);
      if (delayedPriceTimeoutRef.current) clearTimeout(delayedPriceTimeoutRef.current);
      if (soldOutTimeoutRef.current) clearTimeout(soldOutTimeoutRef.current);
          ...
          price !== null && (
            <motion.div
              ...
              className={`flex flex-col items-center p-10 rounded-3xl transition-all duration-500
                ${
                  isFinalMode
                    ? "bg-red-900/40 border-2 border-red-500 shadow-[0_0_50px_rgba(255,0,0,0.6)]"
                    : "bg-black/60 border border-brand-cyan/30 backdrop-blur-md"
                }
                ${triggerShake ? (isFinalMode ? "animate-shake-hard" : "animate-shake") : ""}`}
            >
              <span
                className={`text-[12px] tracking-[0.5em] font-mono mb-4 
                ${
                  isFinalMode
                    ? "text-red-500 shadow-[0_0_10px_#ff0000]"
            ...
            <div className="text-center">
              <motion.h2
                ...
                className={`text-6xl md:text-8xl font-black italic ${
                  isFinalMode
                    ? "text-red-600 drop-shadow-[0_0_40px_#ff0000]"
                    : "text-brand-cyan drop-shadow-[0_0_30px_#00f5ff]"
                }`}
              >
                {currentLevel}
              </motion.h2>
